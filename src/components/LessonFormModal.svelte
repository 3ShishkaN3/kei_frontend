<script>
  ...
  onMount(() => {
    if (editingLesson) {
      ...
      initialTableOfContents = editingLesson.table_of_contents || "";
      tableOfContents = editingLesson.table_of_contents || "";
  ...
  async function getCroppedImg(imageSrc, pixelCrop) {
    ...
    return new Promise((resolve) => {
      canvas.toBlob((blob) => {
        resolve(
          new File([blob], coverImageFile?.name || "cover.png", {
            type: blob.type,
          }),
        );
      }, "image/png");
  ...
  async function handleSubmit() {
    ...
    if (!editingLesson) {
      ...
      formData.append("table_of_contents", tableOfContents.trim());
      changed = true;
    } else {
      ...
      if (tableOfContents.trim() !== initialTableOfContents) {
        formData.append("table_of_contents", tableOfContents.trim());
        changed = true;
      }
    }
...
<div class="modal-overlay">
  <div class="modal-content" on:click|stopPropagation>
    ...
    <form on:submit|preventDefault={handleSubmit}>
      ...
      <div class="form-group">
        <label for="lesson-toc">Оглавление / Описание</label>
        <textarea
          id="lesson-toc"
          bind:value={tableOfContents}
          placeholder="Краткое описание или оглавление урока"
          rows="4"
        ></textarea>
      </div>
      ...
      <div class="form-actions">
        ...
        <button type="submit" class="save-button" disabled={isLoading}
          >{#if isLoading}Сохранение...{:else}{editingLesson
              ? "Сохранить"
              : "Создать"}{/if}</button
        >
      </div>
...
<style>
  ...
  textarea:focus {
    ...
    box-shadow: 0 0 0 3px
      var(--color-input-focus-shadow, rgba(109, 127, 201, 0.2));
  }
