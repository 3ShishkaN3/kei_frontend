<script>
    ...
    function placeSelectedOptionInSlot(slotId) {
        ...
        const itemIndex = draggableItemsPoolForDisplay.findIndex(item => item.id === selectedDraggableOptionForSlot.id);
        if (itemIndex !== -1) {
    ...
    function clearSlot(slotId) {
        ...
        if (removedItem) {
            ...
            draggableItemsPoolForDisplay = [...draggableItemsPoolForDisplay, removedItem];
        }
        ...
        if (viewMode === 'admin' || isTestSubmittedByStudent) {
            ...
            if (isTestSubmittedByStudent && studentSubmission) {
                ...
                if (submissionAnswer) {
                    ...
                    return submissionAnswer.is_correct ? 'student_correct' : 'student_incorrect';
                } else {
...
<div class="drag-drop-test-area">
        ...
        >
            {#each draggableItemsPoolForDisplay as item (item.id)}
                <div class="draggable-option-dnd"
                    title={canStudentInteract ? "Клик для выбора или перетащите: " + item.text : item.text}
                    ...
                    role="button"
                    aria-label={canStudentInteract ? "Нажмите для выбора или перетащите: " + item.text : item.text}
                    class:selected={selectedDraggableOptionForSlot && selectedDraggableOptionForSlot.id === item.id}
    ...
    <div class="drag-drop-slots-container">
        {#each testData.drag_drop_slots || [] as slot (slot.id)}
            ...
            <div class="drag-drop-slot-item"
                 class:status-student_correct={slotDisplayStatus === 'student_correct'}
                 class:status-student_incorrect={slotDisplayStatus === 'student_incorrect'}
                 class:status-slot_empty_after_submit={slotDisplayStatus === 'slot_empty_after_submit'}
            ...
            >
                ...
                <div 
                    ...
                    use:dndzone={{
                        ...
                        type: 'words-pool',
                        dropFromOthersDisabled: false,
                        dropTargetStyle: { outline: '2px dashed #5845d8', backgroundColor: 'rgba(88, 69, 216, 0.1)' }
                    }}
                ...
                >
                    {#each slotItems as item (item.id)}
                        <div class="draggable-option-dnd in-slot" 
                             ...
                             tabindex={canStudentInteract ? 0 : -1}
                             role="button"
                             title={canStudentInteract ? "Удалить облачко из ячейки (клик)" : item.text}
                             animate:flip={{duration: flipDurationMs}}
                        >
                            {item.text}
                        </div>
...
<style>
    ...
    @media (max-width: 768px) {
        ...
        .draggable-item, .draggable-option-dnd { 
            padding: 4px 8px; 
            font-size: 0.8em;
            border-radius: 12px;
        }
    ...
    .slot-prompt :global(.image-item-display), .slot-prompt :global(.audio-item-display-enhanced) { 
        margin-bottom: 5px; 
        border: none; 
        padding: 0; 
        box-shadow: none; 
    ...
    .slot-prompt :global(.image-item-display img) {
        max-width: 100%;
        height: auto;
        max-height: 90px; /* Увеличили с 60px до 90px (в 1.5 раза) */
        object-fit: contain;
    }
    
    @media (max-width: 768px) {
        .slot-prompt :global(.image-item-display img) {
            max-height: 75px; /* Увеличили с 50px до 75px (в 1.5 раза) */
        }
    }
    
    @media (max-width: 480px) {
        .slot-prompt :global(.image-item-display img) {
            max-height: 60px; /* Увеличили с 40px до 60px (в 1.5 раза) */
        }
    }
    ...
    .drop-hint, .drop-placeholder-dnd { font-size: 0.85em; color: #777; font-style: italic; text-align: center; }
    

    
    /* Мобильная адаптация для подсказок */
    @media (max-width: 768px) {
        .drop-hint, .drop-placeholder-dnd { 
            font-size: 0.8em; 
        }
    ...
    .drag-drop-slot-item.status-student_incorrect .draggable-option-dnd.in-slot { 
