<script>
    ...
    function handleDndSequenceFinalize(e) {
        ...
        if (info.source === 'pointer' && info.trigger === 'droppedIntoZone') {
            ...
            wordOrderAvailableOptionsInPool = wordOrderAvailableOptionsInPool.filter(
    ...
    function returnWordFromStudentSequenceToPool(itemIndex) {
        if (!canStudentInteract || isDragging || itemIndex < 0 || itemIndex >= currentWordSequence.length) return;
        const itemToReturn = currentWordSequence[itemIndex];
        currentWordSequence = currentWordSequence.filter((_, i) => i !== itemIndex);
        ...
        >
            {#each wordOrderAvailableOptionsInPool as item (item.id)}
                <div class="draggable-option-dnd word-order-option"
                     ...
                     on:keydown={(e) => { 
                         if (canStudentInteract && !isDragging && (e.key === 'Enter' || e.key === ' ')) {
                             e.preventDefault();
                             addWordToStudentSequenceFromPool(item);
                         }
                     }}
    ...
    >
        {#if currentWordSequence.length > 0}
            {#each currentWordSequence as item, index (item.id)}
                <div class="draggable-option-dnd word-order-option sequence-word-item-dnd"
                ...
                >
                    {#if canStudentInteract} 
                        <DragVertical size="16px" class="drag-handle"/> 
                    {/if}
                    <span>{item.text}</span>
                </div>
            {/each}
...
<style>
    ...
    .empty-pool-message, .empty-sequence-message { 
        width: 100%; 
        text-align: center; 
        font-size: 0.9em; 
        color: #888; 
