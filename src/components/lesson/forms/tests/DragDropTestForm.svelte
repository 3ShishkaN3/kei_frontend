<script>
        ...
        if (!currentTestData || Object.keys(currentTestData).length === 0 || currentTestData.test_type !== 'drag-and-drop') {
            ...
            const slotsCopy = currentTestData.drag_drop_slots
                ? currentTestData.drag_drop_slots.map(s => {
                    const slotId = s.id && typeof s.id !== 'symbol' ? s.id : generateTemporaryId();
                    ...
                    if (slot._currentServerAudioUrl) {
                        ...
                        try { slot._audioFileName = decodeURIComponent(name); } catch(e) { slot._audioFileName = name; }
                    } else {
    ...
    async function getCroppedGeneralTestImage(imageSrc, pixelCrop) {
        ...
        return new Promise((resolve) => {
            canvas.toBlob((blob) => {
                ...
                resolve(new File([blob], testAttachedImageFile?.name || 'cropped_image.png', { type: blob.type || 'image/png' }));
    ...
    function onSlotImageCropChange(slot, e) { slot._imageCrop = e.detail; localTestModel = localTestModel; }
    function onSlotImageZoomChange(slot, e) { slot._imageZoom = e.detail; localTestModel = localTestModel; }
    ...
    async function getCroppedSlotImage(slot, imageSrc, pixelCrop) {
        ...
        return new Promise((resolve) => {
            canvas.toBlob((blob) => {
                if (!blob) {
                    console.error("Не удалось создать Blob из canvas для слота", slot.id);
                    resolve(null); 
                    return; 
                }
                resolve(new File(
    ...
    function handleSlotAudioFileChange(slotToUpdate, event) {
        ...
        if (file) {
            ...
            if (index !== -1) {
                ...
                updatedSlot._imageCroppedAreaPixels = originalSlot._imageCroppedAreaPixels;
                updatedSlot._imageAspectRatio = originalSlot._imageAspectRatio;
    ...
    function removeSlot(index) {
        ...
        if (slotToRemove._imagePreviewUrl && slotToRemove._imagePreviewUrl.startsWith('blob:')) {
    ...
    function validateTest() {
        ...
        if (localTestModel.draggable_options_pool.length === 0) {
            addNotification("Пул опций не может быть пустым. Добавьте варианты для перетаскивания.", "error");
            return false;
        }
        if (localTestModel.drag_drop_slots.length === 0) {
            addNotification("Добавьте хотя бы одну ячейку для ответа (слот).", "error");
            return false;
        }
        for (const slot of localTestModel.drag_drop_slots) {
            if (!slot.correct_answer_text) {
                addNotification("Каждый слот должен иметь выбранный правильный ответ из пула.", "error");
                return false;
            }
        }
        return true;
    ...
    async function handleSave() {
        ...
        if (finalGeneralImageFile) {
            ...
            console.log("[DragDropForm] handleSave: Добавлен общий файл изображения:", finalGeneralImageFile.name);
        }
        ...
        console.log("[DragDropForm] handleSave: JSON 'test_definition' добавлен:", JSON.stringify(testDefinition, null, 2));
        ...
        for (let i = 0; i < localTestModel.drag_drop_slots.length; i++) {
            ...
            if (audioFileFromMap) {
                slotAudioFilesToAttach.push(audioFileFromMap);
                 console.log(`[DragDropForm] handleSave: Слот ${i + 1} (ID: ${slot.id}) - файл аудио ${audioFileFromMap.name} добавлен в очередь.`);
            }
        }
    ...
    onDestroy(() => {
        ...
        localTestModel.drag_drop_slots.forEach(slot => {
            if (slot._imagePreviewUrl && slot._imagePreviewUrl.startsWith('blob:')) {
            ...
            <div class="slots-list">
                {#each localTestModel.drag_drop_slots as slot, index (slot.id)}
                    <div class="slot-item" transition:slide|local>
                        ...
                        <div class="form-row">
                            <div class="form-group attachment-control">
                                <label for={"slot-image-upload-" + index + "-" + slot.id}>Изображение для задания</label>
                                {#if slot._imagePreviewUrl}
                                    <div class="cropper-wrapper-test slot-cropper">
                                        <Cropper
                                            image={slot._imagePreviewUrl}
                                            crop={slot._imageCrop}
                                            zoom={slot._imageZoom}
                                            aspect={slot._imageAspectRatio}
                                            on:cropchange={(e) => onSlotImageCropChange(slot, e)}
                                            on:zoomchange={(e) => onSlotImageZoomChange(slot, e)}
                                            on:cropcomplete={(e) => onSlotImageCropComplete(slot, e)}
                                            on:wheel={(e) => onSlotImageZoomChange(slot, { detail: { zoom: slot._imageZoom + e.deltaY * -0.001 } })}
                                            cropShape="rect"
                                            showGrid={true}
                                        />
                                    </div>
                                    <div class="attachment-actions">
                                        <div class="aspect-ratio-controls">
                                            ...
                                            <button
                                                type="button" class="aspect-btn"
                                                class:active={slot._imageAspectRatio === ar.value && !(slot._imageAspectRatio === null && ar.value === null && slot._imageAspectRatio !== ar.value) }
                                                on:click={() => { slot._imageAspectRatio = ar.value; localTestModel = localTestModel; }}
                                                title={ar.label}
                                            >{ar.label}</button>
                                            {/each}
                                        </div>
                                        <label class="file-upload-label small" for={"slot-image-replace-" + index + "-" + slot.id}>
                                            <ImagePlusOutline size="18px"/><span>Заменить</span>
                                        </label>
                                        <input type="file" id={"slot-image-replace-" + index + "-" + slot.id} class="visually-hidden" accept="image/*" on:change={(e) => handleSlotImageFileChange(slot, e)} disabled={isLoading}/>
                                        <button type="button" class="remove-attachment-btn small" on:click={() => removeSlotAttachedImage(slot)} title="Удалить изображение">
                                            <Close size="18px"/> Удалить
                                        </button>
                                    </div>
                                {:else}
                                    <label class="file-upload-label main-upload-trigger" for={"slot-image-upload-input-" + index + "-" + slot.id}>
                                        <ImagePlusOutline size="24px"/>
                                        <span>{slot._currentServerImageId ? `ID: ${slot._currentServerImageId}. Загрузить новый.` : 'Загрузить изображение'}</span>
                                    </label>
                                    <input type="file" id={"slot-image-upload-input-" + index + "-" + slot.id} class="visually-hidden" accept="image/*" on:change={(e) => handleSlotImageFileChange(slot, e)} disabled={isLoading}/>
                                {/if}
                                {#if slot._currentServerImageId && !slot._imagePreviewUrl && !slot._attachedImageFile}
                                    <small class="form-hint removed-hint">Изображение (ID: {slot._currentServerImageId}) будет удалено.</small>
                                {/if}
                            </div>
                            <div class="form-group attachment-control">
                                <label for={"slot-audio-upload-" + index + "-" + slot.id}>Аудио для задания</label>
                                {#if slot._audioFileName}
                                    <div class="audio-filename-display">
                                        <span>{slot._audioFileName}</span>
                                        <button type="button" class="remove-attachment-btn small" on:click={() => removeSlotAttachedAudio(slot)} title="Удалить аудио">
                                            <Close size="18px"/>
                                        </button>
                                    </div>
                                {/if}
                                <label class="file-upload-label main-upload-trigger" class:hidden={!!slot._audioFileName} for={"slot-audio-upload-input-" + index + "-" + slot.id}>
                                    <MusicNotePlus size="24px"/>
                                    <span>{slot._currentServerAudioId ? `ID: ${slot._currentServerAudioId}. Загрузить новый.` : 'Загрузить аудио'}</span>
                                </label>
                                <input type="file" id={"slot-audio-upload-input-" + index + "-" + slot.id} class="visually-hidden" accept="audio/*" on:change={(e) => handleSlotAudioFileChange(slot, e)} disabled={isLoading}/>
                                {#if slot._currentServerAudioId && !slot._audioFileName && !slot._attachedAudioFile}
                                    <small class="form-hint removed-hint">Аудио (ID: {slot._currentServerAudioId}) будет удалено.</small>
                                {/if}
                            </div>
                        </div>
                        <div class="form-group">
...
.cropper-wrapper-test.slot-cropper { position: relative; width: 100%; height: 200px; background: #eef2f7; border-radius: var(--spacing-border-radius-small); overflow: hidden; margin-bottom: 10px; border: 1px solid var(--color-border-light); }
