<script>
  ...
  import { MCQTestModel, MCQOptionModel } from '../../../../models/testTypes.js';
  ...
  let localTestModel = new MCQTestModel({ title: '', test_type: 'mcq-single', mcq_options: [] });
  ...
  function initializeModel(currentTestData) {
      if (!currentTestData) {
          localTestModel = new MCQTestModel({ title: '', test_type: 'mcq-single', mcq_options: [] });
      } else {
          const optionsCopy = currentTestData.mcq_options 
              ? currentTestData.mcq_options.map(opt => new MCQOptionModel({
                  ...opt, 
                  id: opt.id && typeof opt.id !== 'symbol' ? opt.id : generateTemporaryId() 
                })) 
              : [];
          localTestModel = new MCQTestModel({ ...currentTestData, mcq_options: optionsCopy });
      }

      isMultiSelect = localTestModel.test_type === 'mcq-multi';
      
      currentServerImageId = currentTestData?.attached_image_id || null;
      currentServerImageUrl = currentTestData?.attached_image_details?.image || null;
      if (currentServerImageUrl && !attachedImageFile) {
          imagePreviewUrlForCropper = currentServerImageUrl.startsWith('http') ? currentServerImageUrl : API_BASE_URL + currentServerImageUrl;
      ...
      const arFromData = currentTestData?.aspect_ratio_for_test_image; // Предполагаем такое поле от бэка
      const foundAr = aspectRatios.find(ar => ar.value === arFromData);
      imageAspectRatio = foundAr ? foundAr.value : null;


      currentServerAudioId = currentTestData?.attached_audio_id || null;
      currentServerAudioUrl = currentTestData?.attached_audio_details?.audio_file || null;
      if (currentServerAudioUrl && !attachedAudioFile) {
          const name = currentServerAudioUrl.substring(currentServerAudioUrl.lastIndexOf('/') + 1);
          try { audioFileName = decodeURIComponent(name); } catch(e) { audioFileName = name; }
      } else if (!attachedAudioFile) {
          audioFileName = null;
      }

      if ((!localTestModel.mcq_options || localTestModel.mcq_options.length === 0) && currentTestData?.title !== undefined) {
  ...
  $: if (testData) {
      const currentTestDataString = JSON.stringify({
          title: testData.title, description: testData.description, test_type: testData.test_type,
          attached_image_id: testData.attached_image_id, attached_audio_id: testData.attached_audio_id,
  ...
  function addOptionInternal(isCorrect = false) {
      ...
      if (!isMultiSelect && isCorrect) {
          localTestModel.mcq_options.forEach(opt => opt.is_correct = false);
      }
      localTestModel.addOption({ text: '', is_correct: actualCorrect, explanation: '', order: newOrder, id: generateTemporaryId() });
  }
  ...
  function removeOption(index) {
      if (!localTestModel || localTestModel.mcq_options.length <= 1) {
          addNotification("Тест должен иметь хотя бы одну опцию ответа.", "warning"); return;
      }
  ...
  function handleOptionCorrectChange(selectedIndex) {
      ...
      if (!isMultiSelect) {
          ...
          localTestModel.mcq_options[selectedIndex].is_correct = !localTestModel.mcq_options[selectedIndex].is_correct;
      }
      localTestModel = localTestModel;
  }
  ...
  function handleImageFileChange(event) {
      ...
      if (file) {
          ...
          if (imagePreviewUrlForCropper && imagePreviewUrlForCropper.startsWith('blob:')) URL.revokeObjectURL(imagePreviewUrlForCropper);
          imagePreviewUrlForCropper = URL.createObjectURL(file);
          showImageCropper = true;
          imageCrop = { x: 0, y: 0 }; imageZoom = 1; imageCroppedAreaPixels = null;
      }
      event.target.value = null; 
  }
  function onImageTestCropComplete(e) { imageCroppedAreaPixels = e.detail.pixels; }
  ...
  async function getCroppedTestImage(imageSrc, pixelCrop) {
      ...
      try {
          ...
          addNotification("Ошибка загрузки изображения для обрезки в тесте.", "error"); return null;
      }
      ...
      return new Promise((resolve) => {
          canvas.toBlob((blob) => {
              ...
              resolve(new File([blob], attachedImageFile?.name || 'test-cropped-image.png', { type: blob.type || 'image/png' }));
          }, attachedImageFile?.type || 'image/png', 0.9);
      });
  }
  ...
  function validateTest() {
      ...
      if (!localTestModel.title.trim()) {
          addNotification("Введите название теста.", "error"); return false;
      }
      if (!localTestModel.mcq_options || localTestModel.mcq_options.length === 0) {
          addNotification("Тест должен содержать хотя бы одну опцию ответа.", "error"); return false;
      }
      if (localTestModel.mcq_options.some(opt => !opt.text.trim())) {
          addNotification("Все опции ответа должны содержать текст.", "error"); return false;
      }
      if (!isMultiSelect && !localTestModel.mcq_options.some(opt => opt.is_correct)) {
          addNotification("Для теста с одним вариантом ответа должен быть выбран правильный ответ.", "error"); return false;
      }
      return true;
  }
  ...
  async function handleSave() {
      ...
      if (attachedImageFile && showImageCropper) {
          if (imageCroppedAreaPixels && imageCroppedAreaPixels.width > 0 && imageCroppedAreaPixels.height > 0) {
              finalCroppedImageFile = await getCroppedTestImage(imagePreviewUrlForCropper, imageCroppedAreaPixels);
          } else {
      ...
      const testDefinition = {
          ...
          mcq_options: localTestModel.mcq_options.map(opt => {
              ...
              if (typeof payloadOpt.id === 'string' && payloadOpt.id.startsWith('temp_')) {
  ...
  onDestroy(() => {
      if (imagePreviewUrlForCropper && imagePreviewUrlForCropper.startsWith('blob:')) {
...
<div class="item-form mcq-test-form">
  {#if localTestModel}
      <div class="form-group">
          <label for={"mcq-test-title-" + (localTestModel.id && typeof localTestModel.id !== 'symbol' ? localTestModel.id : 'new')}>Название теста</label>
          <input type="text" id={"mcq-test-title-" + (localTestModel.id && typeof localTestModel.id !== 'symbol' ? localTestModel.id : 'new')} bind:value={localTestModel.title} placeholder="Введите название теста" disabled={isLoading} />
      </div>

      <div class="form-group">
          <label for={"mcq-test-description-" + (localTestModel.id && typeof localTestModel.id !== 'symbol' ? localTestModel.id : 'new')}>Описание/Инструкция</label>
          <textarea id={"mcq-test-description-" + (localTestModel.id && typeof localTestModel.id !== 'symbol' ? localTestModel.id : 'new')} bind:value={localTestModel.description} rows="3" placeholder="Описание или инструкция (необязательно)" disabled={isLoading}></textarea>
      </div>

      <div class="form-group form-group-custom-checkbox">
          <input 
              type="checkbox" 
              id={"mcq-is-multi-" + (localTestModel.id && typeof localTestModel.id !== 'symbol' ? localTestModel.id : 'new')} 
              class="custom-checkbox-input"
              bind:checked={isMultiSelect} 
              disabled={isLoading} 
          />
          <label for={"mcq-is-multi-" + (localTestModel.id && typeof localTestModel.id !== 'symbol' ? localTestModel.id : 'new')} class="custom-checkbox-label">
              <span class="checkbox-visual">
                  {#if isMultiSelect} <CheckboxMarked size="18px" /> {:else} <CheckboxBlankOutline size="18px" /> {/if}
              </span>
      ...
      <div class="attachments-section">
          ...
          <div class="form-row">
              <div class="form-group attachment-control">
                  <label for={"mcq-image-upload-input-" + (localTestModel.id && typeof localTestModel.id !== 'symbol' ? localTestModel.id : 'new')}>Изображение к тесту</label>
                  {#if showImageCropper && imagePreviewUrlForCropper}
                      <div class="cropper-wrapper-test">
                           <Cropper
                              ...
                              on:cropchange={(e) => imageCrop = e.detail}
                              on:zoomchange={(e) => imageZoom = e.detail}
                      ...
                      <div class="attachment-actions">
                          <div class="aspect-ratio-controls">
                              ...
                              <button 
                                  type="button" 
                                  class="aspect-btn" 
                                  class:active={imageAspectRatio === ar.value && !(imageAspectRatio === null && ar.value === null && imageAspectRatio !== ar.value) } 
                                  on:click={() => imageAspectRatio = ar.value}
                                  title={ar.label}
                              >{ar.label}</button>
                              {/each}
                          </div>
                          <label class="file-upload-label small" for={"mcq-image-upload-replace-" + (localTestModel.id && typeof localTestModel.id !== 'symbol' ? localTestModel.id : 'new')}>
                              <ImagePlusOutline size="18px"/>
                              <span>Заменить</span>
                          </label>
                          <input type="file" id={"mcq-image-upload-replace-" + (localTestModel.id && typeof localTestModel.id !== 'symbol' ? localTestModel.id : 'new')} class="visually-hidden" accept="image/*" on:change={handleImageFileChange} disabled={isLoading}/>
                          <button type="button" class="remove-attachment-btn small" on:click={removeAttachedImage} title="Удалить изображение">
                              <CloseCircle size="18px"/> Удалить
                          </button>
                      </div>
                  {:else}
                      <label class="file-upload-label main-upload-trigger" for={"mcq-image-upload-input-" + (localTestModel.id && typeof localTestModel.id !== 'symbol' ? localTestModel.id : 'new')}>
                          <ImagePlusOutline size="24px"/>
                          <span>{currentServerImageId ? `Текущее ID: ${currentServerImageId}. Загрузить новый для замены.` : 'Загрузить изображение'}</span>
                      </label>
                      <input type="file" id={"mcq-image-upload-input-" + (localTestModel.id && typeof localTestModel.id !== 'symbol' ? localTestModel.id : 'new')} class="visually-hidden" accept="image/*" on:change={handleImageFileChange} disabled={isLoading}/>
                  {/if}
                   {#if currentServerImageId && !showImageCropper && !attachedImageFile}
                      <small class="form-hint removed-hint">Изображение (ID: {currentServerImageId}) будет удалено при сохранении.</small>
                  {/if}
              </div>

              <div class="form-group attachment-control">
                  <label for={"mcq-audio-upload-input-" + (localTestModel.id && typeof localTestModel.id !== 'symbol' ? localTestModel.id : 'new')}>Аудио к тесту</label>
                  {#if audioFileName}
                      <div class="audio-filename-display">
                          <span>{audioFileName}</span>
                          <button type="button" class="remove-attachment-btn small" on:click={removeAttachedAudio} title="Удалить аудио">
                              <CloseCircle size="18px"/>
                          </button>
                      </div>
                  {/if}
                   <label class="file-upload-label main-upload-trigger" class:hidden={!!audioFileName} for={"mcq-audio-upload-input-" + (localTestModel.id && typeof localTestModel.id !== 'symbol' ? localTestModel.id : 'new')}>
                      <MusicNotePlus size="24px"/>
                      <span>{currentServerAudioId ? `Текущее ID: ${currentServerAudioId}. Загрузить новый для замены.` : 'Загрузить аудио'}</span>
                  </label>
                  <input type="file" id={"mcq-audio-upload-input-" + (localTestModel.id && typeof localTestModel.id !== 'symbol' ? localTestModel.id : 'new')} class="visually-hidden" accept="audio/*" on:change={handleAudioFileChange} disabled={isLoading}/>
                  {#if currentServerAudioId && !audioFileName && !attachedAudioFile}
                       <small class="form-hint removed-hint">Аудио (ID: {currentServerAudioId}) будет удалено при сохранении.</small>
                  {/if}
              </div>
          </div>
      </div>
      ...
      {#if localTestModel.mcq_options}
          {#each localTestModel.mcq_options as option, index (option.id)}
              <div class="mcq-option-item" transition:fade|local={{duration:150}}>
                  <label class="option-correct-toggle-label" title={option.is_correct ? "Отмечен как правильный" : "Отметить как правильный"}>
                      <input 
                          type="checkbox" 
                          class="custom-checkbox-input"
                          ...
                          disabled={isLoading}
                          aria-labelledby={"mcq-option-text-label-" + index + "-" + option.id}
                      />
                      <span class="checkbox-visual large">
                           {#if option.is_correct} <CheckboxMarked size="22px" /> {:else} <CheckboxBlankOutline size="22px" /> {/if}
                      </span>
                  </label>
                  <div class="option-inputs">
                      <div class="form-group option-text-group">
                          <label for={"mcq-option-text-" + index + "-" + option.id} id={"mcq-option-text-label-" + index + "-" + option.id}>Текст опции #{index + 1}</label>
                          <input type="text" id={"mcq-option-text-" + index + "-" + option.id} bind:value={option.text} placeholder="Текст варианта ответа" disabled={isLoading} />
                      </div>
                      <div class="form-group option-explanation-group">
                          <label for={"mcq-option-explanation-" + index + "-" + option.id}>Пояснение (необязательно)</label>
                          <input type="text" id={"mcq-option-explanation-" + index + "-" + option.id} bind:value={option.explanation} placeholder="Пояснение к варианту" disabled={isLoading} />
                      </div>
                  </div>
                  <button 
                      type="button" 
                      ...
                      title="Удалить опцию"
                      disabled={isLoading || localTestModel.mcq_options.length <= 1}
                  >
      ...
      <button type="button" class="add-option-btn" on:click={handleAddOptionClick} disabled={isLoading}>
...
.attachment-control > label:first-child { font-size: 0.95rem; margin-bottom: 8px; }
...
.cropper-wrapper-test { position: relative; width: 100%; height: 220px; background: #eef2f7; border-radius: var(--spacing-border-radius-small); overflow: hidden; margin-bottom: 10px; border: 1px solid var(--color-border-light); }
.attachment-actions { display: flex; justify-content: space-between; align-items: center; margin-top: 8px; gap: 10px; }
.aspect-ratio-controls { display: flex; gap: 5px; align-items: center; color: var(--color-text-muted); }
.aspect-ratio-controls > span { margin-right: 5px; }
.aspect-btn { font-size: 0.75em; padding: 3px 6px; border: 1px solid #ccc; background: #f9f9f9; border-radius: 4px; cursor: pointer; }
...
.remove-attachment-btn:hover { background-color: rgba(var(--color-danger-red-rgb), 0.1); color: var(--color-danger-red); border-color: rgba(var(--color-danger-red-rgb),0.3); }
...
.audio-filename-display span { font-size: 0.9em; color: var(--color-text-dark); word-break: break-all; }
...
.option-correct-toggle-label { margin-top: 28px; display: inline-flex; align-items: center; cursor: pointer; }
.option-correct-toggle-label .checkbox-visual.large { width: 24px; height: 24px; }
.option-correct-toggle-label .checkbox-visual.large :global(svg) { width: 20px; height: 20px; }
.option-inputs { flex-grow: 1; display: flex; flex-direction: column; gap: 10px; }
.option-text-group, .option-explanation-group { margin-bottom: 0; }
.option-delete-btn { background: none; border: none; cursor: pointer; padding: 5px; color: var(--color-text-muted); margin-left: auto; align-self: center; transition: color 0.2s; }
.option-delete-btn:hover:not(:disabled) { color: var(--color-danger-red); }
.option-delete-btn:disabled { color: #ccc; cursor: not-allowed;}
