<script>
    ...
    onMount(() => {
        if (itemToEdit && itemToEdit.content_details) {
            ...
            current_image_url_from_server = itemToEdit.content_details.image_url;
            if (current_image_url_from_server) {
                image_for_cropper = current_image_url_from_server.startsWith('http')
    ...
    async function getCroppedImg(imageSrc, pixelCrop) {
        ...
        try {
            ...
            addNotification("Ошибка загрузки изображения для обрезки.", "error");
            return null;
        }
...
<form on:submit|preventDefault={handleSubmit} class="item-form">
    ...
    <div class="form-group">
        <label for="image-alt-text">Альтернативный текст (для доступности)</label>
        <input type="text" id="image-alt-text" bind:value={alt_text} disabled={isLoading} />
    </div>
        ...
        <div class="image-upload-container">
            {#if showCropper && image_for_cropper}
                <div class="aspect-ratio-selector">
                    ...
                    <button type="button" class:active={currentAspect === 1/1} on:click={() => setAspect(1/1)}>1:1</button>
                    <button type="button" class:active={currentAspect === 16/9} on:click={() => setAspect(16/9)}>16:9</button>
                    <button type="button" class:active={currentAspect === 4/3} on:click={() => setAspect(4/3)}>4:3</button>
                    <button type="button" class:active={currentAspect === 3/4} on:click={() => setAspect(3/4)}>3:4</button> 
                </div>
                ...
                <div class="cropper-controls">
                     <label class="change-image-label">
                        <input type="file" accept="image/*" on:change={handleFileChange} disabled={isLoading} style="display:none;" />
                        Изменить файл
                    </label>
                    <button type="button" class="remove-image-btn-inline" on:click={removeImage} title="Удалить текущее изображение">
            ...
            {:else}
                ...
                <input type="file" id="image-file-input-img" accept="image/*" on:change={handleFileChange} disabled={isLoading} style="display:none;" />
            {/if}
        </div>
    </div>
    ...
    <div class="form-actions">
        <button type="submit" class="btn-save" 
            disabled={isLoading || (!itemToEdit && !image_file_original && !showCropper) || (showCropper && !image_for_cropper && !current_image_url_from_server) }>
            {isLoading ? 'Сохранение...' : (itemToEdit ? 'Обновить изображение' : 'Добавить изображение')}
        </button>
...
<style>
    ...
    .image-upload-label-box span { margin-top: 8px; font-size: 0.9em; }
    ...
    .btn-save:disabled { background-color: #ccc; opacity: 0.7; cursor: not-allowed;}
    .btn-cancel {
    ...
    .aspect-ratio-selector label { margin-bottom: 0; margin-right: 5px; }
