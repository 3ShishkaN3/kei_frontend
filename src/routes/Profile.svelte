<script>
  ...
  async function requestEmailChange() {
    ...
    try {
      ...
      const response = await apiFetch(
        `${API_BASE_URL}/auth/email/change/request/`,
        {
          method: "POST",
      ...
      if (response.ok) {
        ...
        emailChangeSuccess =
          data.message || "Код подтверждения отправлен на новый email.";
      ...
      console.error(err);
      emailChangeError =
        "Ошибка соединения при отправке запроса на смену почты";
    }
  }
  ...
  async function confirmEmailChange() {
    ...
    try {
      ...
      const response = await apiFetch(
        `${API_BASE_URL}/auth/email/change/confirm/`,
        {
          method: "POST",
          body: JSON.stringify(payload),
        },
  ...
  async function requestPasswordChange() {
    ...
    try {
      ...
      const response = await apiFetch(
        `${API_BASE_URL}/auth/password/change/request/`,
        {
          method: "POST",
          body: JSON.stringify(payload),
        },
      ...
      if (response.ok) {
        const data = await response.json();
        passwordChangeSuccess =
          data.message || "Код подтверждения отправлен на вашу почту.";
      ...
      console.error(err);
      passwordChangeError =
        "Ошибка соединения при отправке запроса на смену пароля";
    }
  }
  ...
  async function confirmPasswordChange() {
    ...
    try {
      ...
      const response = await apiFetch(
        `${API_BASE_URL}/auth/password/change/confirm/`,
        {
          method: "POST",
          body: JSON.stringify(payload),
        },
  ...
  function handleVkPaste(event) {
    event.preventDefault();
    const pastedText = event.clipboardData.getData('text');
    vkLinkRest = cleanUrl(pastedText);
  }

  function handleTelegramPaste(event) {
    event.preventDefault();
    const pastedText = event.clipboardData.getData('text');
    telegramLinkRest = cleanUrl(pastedText);
  }
  ...
  async function fetchProfile() {
    try {
      ...
      if (response.ok) {
        ...
        profileData.avatar = profileData.avatar || "/avatar.jpg";
        avatar.set(profileData.avatar);

        phoneInfo = profileData.phone_number || "";

        vkLinkRest =
          profileData.vk_link && profileData.vk_link.startsWith("https://")
            ? profileData.vk_link.slice(8)
            : profileData.vk_link && profileData.vk_link.startsWith("http://")
            ? profileData.vk_link.slice(7)
            : profileData.vk_link || "";
        telegramLinkRest =
          profileData.telegram_link &&
          profileData.telegram_link.startsWith("https://")
            ? profileData.telegram_link.slice(8)
            : profileData.telegram_link &&
            profileData.telegram_link.startsWith("http://")
            ? profileData.telegram_link.slice(7)
            : profileData.telegram_link || "";
  ...
  async function savePersonalInfo() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        personalInfoError =
          data.detail || "Ошибка сохранения персональной информации";
      }
    } catch (err) {
      console.error(err);
      personalInfoError =
        "Ошибка соединения при сохранении персональной информации";
    }
  }
    ...
    try {
      ...
      if (response.ok) {
        try {
          ...
          profileData.avatar = `${result.avatar || "/avatar.jpg"}`;
          avatar.set(profileData.avatar);
...
<svelte:head>
  ...
  <meta
    name="description"
    content="Личные данные, безопасность и предпочтения отображения."
  />
</svelte:head>
...
<main>
  ...
  <section class="profile-block">
    ...
    <div class="avatar-container">
      ...
      {#if profileData.avatar && profileData.avatar !== "/avatar.jpg"}
  ...
  <section class="block">
    ...
    {#if !showEmailConfirmation}
      ...
      <div class="btn-container">
        <button class="save-btn" on:click={requestEmailChange}
          >Сменить почту</button
        >
      </div>
    {:else}
      <div class="form-group">
      ...
      <div class="btn-container">
        <button class="save-btn" on:click={confirmEmailChange}
          >Подтвердить</button
        >
        <button
          class="cancel-btn"
          on:click={() => (showEmailConfirmation = false)}>Отмена</button
        >
      </div>
    {/if}
  </section>
  ...
  <section class="block">
    ...
    {#if !showPasswordConfirmation}
      ...
      <div class="btn-container">
        <button class="save-btn" on:click={requestPasswordChange}
    ...
    {:else}
      ...
      <div class="btn-container">
        <button class="save-btn" on:click={confirmPasswordChange}
          >Подтвердить</button
        >
        <button
          class="cancel-btn"
          on:click={() => (showPasswordConfirmation = false)}>Отмена</button
        >
      </div>
    {/if}
  </section>
  ...
  <section class="block">
    ...
    <div class="settings-item">
      <label class="switch-label" for="showAnswersToggle">
        ...
        />
        <span class="switch-slider"></span>
        <p class="switch-text">Отображение ответов выполненных тестов</p>
      </label>
    </div>
  </section>
</main>
...
<style>
  ...
  .avatar {
    ...
    border: var(--spacing-avatar-border-width-profile) solid
      var(--color-avatar-border-profile);
    ...
    box-shadow: 0 2px var(--spacing-avatar-shadow-blur-profile)
      var(--color-avatar-shadow-profile);
  }
  ...
  .switch-text {
    margin-left: var(--spacing-settings-label-gap);
  }
